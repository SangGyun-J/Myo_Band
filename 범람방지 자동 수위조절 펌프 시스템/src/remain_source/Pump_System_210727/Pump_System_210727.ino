...
void setup() {
   ...
   // Firebase.begin(FIREBASE_HOST,FIREBASE_Authorization_key);
    ...
    pcf20.pinMode(App_button1, INPUT);
    pcf20.pinMode(App_button2, INPUT);
    ...
    //버튼값 초기화
    pcf20.digitalWrite(App_button1, LOW);
    pcf20.digitalWrite(App_button2, LOW);
    ...
    while (client.connected()){
      if (client.available()){
        ...
        if (request.indexOf("/Auto=ON") != -1)  {  
          ...
          pcf20.digitalWrite(App_button1, HIGH); 
          Auto = HIGH;          
        }
        ...
        if (request.indexOf("/Pump=ON") != -1)  {
          ...
          pcf20.digitalWrite(App_button2, HIGH); 
          Pump = HIGH;             
        }
